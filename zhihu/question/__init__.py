        ...
        try:
            ...
            for answer_content in database:
                ...
                try:
                    ...
                    open(r'E:\Development\Python\Project\ZhihuSpider 4.0/error.html',
                         'w', encoding='utf8').write(content.prettify())
                    raise ParseError('发生了一个解析错误，请查看错误信息和error.html文件。')
...
def answer_msg(answer_content):
    ...
    answer_dict['author'] = answer_content['author']['name']
    answer_dict['voteup'] = answer_content['voteup_count']
    answer_dict['title'] = answer_content['question']['title']
    question_id = answer_content['question']['id']
    answer_id = answer_content['id']
    answer_dict['original_url'] = const.ANSWER_URL.format(question_id, answer_id)
    answer_dict['author_page'] = const.AUTHOR_PAGE_URL.format(answer_content['author']['url_token'])
    answer_dict['author_avatar_url'] = \
        answer_content['author']['avatar_url_template'].replace(
            const.AVATAR_SIZE_R, const.AVATAR_SIZE_A)
    answer_dict['created_date'] = timer.timestamp_to_date(answer_content['created_time'])
    ...
    while not offset.is_end():
        ...
        for answer_content in database:
            ...
            try:
                ...
                open(r'E:\Development\Python\Project\ZhihuSpider 4.0/error.html',
                     'w', encoding='utf8').write(content.prettify())
                raise ParseError('解析错误，请查看文件error.html')
        timer.sleep_for(zhihu.SLEEP)
