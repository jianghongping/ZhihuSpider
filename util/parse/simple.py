        ...
        if self.support.get(self.element_tag.name, '') != 'a':
            ...
            try:
                ...
                if self.element_tag.get('data-draft-type', None) == "mcn-link-card":
                    ...
                    print(element_tag)
                    # TODO bug in here with question id 311297911
                    raise ParseError()
...
类。即内容单一，不能再细分的元素。如图片类（figure）、视频类（Video）、代码类（Code），这些类都是可以直接解析成markdown
元素的基本元素。Multilevel类是所有复合类型的基类，这个基类也继承了Simple类。比如说一个p标签就是一个Paragraph类，p标签可
